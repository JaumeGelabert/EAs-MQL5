...
//---
   ...
   if(PositionsTotal() == 0)
   ...
   else
     {
      ...
      double step = 0; //--- Step
      double slBuy  = NormalizeDouble(Ask-0.0005,_Digits); //--- SL Buy
      double slSell = NormalizeDouble(Bid+0.0005,_Digits); //--- SL Sell
         ...
         if(Bid > positionPriceOpen && ((positionSL + step) < slBuy))  //--- Si Bid es mayor al precio de entrada, y SL en mercado + 'step' es menor al SL*,...
           {
            ...
            OrderSend(request,result); //--- Enviamos operacion modificada
           }
        }
      else
