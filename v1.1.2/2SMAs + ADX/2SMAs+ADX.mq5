...
//--- Arrays de SMAs para los tres periodos
   double SMA_Array_A[],SMA_Array_B[];
   ...
   if(PositionsTotal() == 0) //--- Solo tomamos una operación a la vez. Si ya hay una posición habierta, no abrimos mas.
     {
      if(Digits() == 5) //--- 5 DIGITOS
        {
         if((((SMA_Array_A[0]<SMA_Array_B[0]) && SMA_Array_A[1]>SMA_Array_B[1])) && (Valor_ADX > Valor_ADX_Necesario))   //--- SELL
           {
         ...
         if(((SMA_Array_A[0]>SMA_Array_B[0]) && SMA_Array_A[1]<SMA_Array_B[1]) && (Valor_ADX > Valor_ADX_Necesario)) //--- BUY
           {
