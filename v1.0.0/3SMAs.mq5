...
input double RiesgoPorOperacion = 0.02; //--- Expresar en base 1. Necesario para <optimalLotSize.mqh>
...
//--- 5 digitos
input double SL_5digitos = 0.00250; //--- Tambien se tiene en cuenta en <optimalLotSize.mqh>
...
input double ProfitParaTrailSL = 20;
   ...
   if(PositionsTotal() == 0) //--- Solo tomamos una operación a la vez. Si ya hay una posición habierta, no abrimos mas.
     {
      if(Digits() == 5) //--- 5 DIGITOS
        {
         if(((SMA_Array_A[0]<SMA_Array_B[0]) && (SMA_Array_C[0]<SMA_Array_A[0])) && SMA_Array_A[1]>SMA_Array_B[1]) //--- SELL
           {
            ...
            if(SpreadInPips_5Digits < spreadMaximo) //--- Podemos operar. Spread dentro de las condiciones aceptadas.
              {
               ...
               request.sl = SL_5digitos;
               request.tp = TP_5digitos;
         ...
         if(((SMA_Array_A[0]>SMA_Array_B[0]) && (SMA_Array_C[0]>SMA_Array_A[0])) && SMA_Array_A[1]<SMA_Array_B[1]) //--- BUY
           {
